...
\begin{itemize}[nosep]
  ...
  \item Remaining columns: non-negative integer counts for microbial features. No NAs.
  The sum of counts per sample is used as sequencing depth by default. Therefore it must be positive for all samples.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item This example does not include a sequencing depth 
    ...
    If \texttt{offsetCol} is not specified, the pipeline uses automatically calculated sequencing depth.
...
\begin{verbatim}
SNP	SCORE	VAR	PVAL
chr1.1119172.G.A	-7.46170816152792	114.082683798442	0.484802231639899
chr1.1423343.G.T	15.6764456780156	82.3399795446402	0.0840606557169883
chr1.3522693.A.G	-2.69387870487415	24.0399452130804	0.582711318455373
\end{verbatim}

\paragraph{\texttt{palm2\_step2\_chr1\_g1.txt}}
\begin{verbatim}
SNP	SCORE	VAR	PVAL
chr1:1119172:G:A	-1.28953	3.62478	0.498206
chr1:1423343:G:T	-3.46874	4.63856	0.107273
chr1:3522693:A:G	2.18819	4.6194	0.308628
\end{verbatim}

Columns: \texttt{SNP} (variant id), \texttt{SCORE} (score statistic),
\texttt{VAR} (variance of the score), and \texttt{PVAL} (single-variant p-value).


\section{\texttt{quick.sh}: parameter block}

The script \texttt{quick.sh} is the main user interface. 
\texttt{quick\_docker.sh} is a modified version for Docker usage.
...
\begin{itemize}[nosep]
  \item We prefer using automatically generated GRM by step 0. 
        If you want to use pre-computed GRM, place the rds file
        under \texttt{input/} and set \texttt{grmFile} accordingly.
        Make sure your own GRM rds file has the same structure as the 
        one automatically generated by step 0. 
        Please contact the author for assistance on the structure.
  ...
  \item This document focuses on quick-start shell wrappers
        ...
        please contact the author for assistance on deployment.
\end{itemize}
